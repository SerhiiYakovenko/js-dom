...
function createItem(item) {
    ...
    const mainItemContent = createElementWithClassName('p', item.text, 'main__content');
    div.appendChild(mainItemContent);
    const mainItemImage = createElementWithClassName('img', item.img, 'main__image');
...
for (var i = 0; i < navSection.length; i++) {
    navSection[i].addEventListener('click', function () {
        hideItems(); 
        document.getElementsByClassName(this.textContent.toLowerCase().split(' ').join('_'))[0].classList.remove('hidden');
    });
}

function hideItems(){
    let arrayOfElements = Array.from(document.getElementsByClassName('main__item'));
